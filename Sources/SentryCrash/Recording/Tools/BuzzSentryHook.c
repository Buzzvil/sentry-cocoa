...
static size_t
sentrycrash__thread_idx(BuzzSentryCrashThread thread)
{
    // This uses the same magic numbers as FNV, but rather follows the simpler
    // `(x * b + c) mod n` hashing scheme. Also note that `BuzzSentryCrashThread`
...
sentrycrash_get_async_caller_for_thread(BuzzSentryCrashThread thread)
{
...
{
    BuzzSentryCrashThread thread = sentrycrashthread_self();

    size_t idx = sentrycrash__thread_idx(thread);
    sentrycrash_async_caller_slot_t *slot = &sentry_async_callers[idx];

    BuzzSentryCrashThread expected = (BuzzSentryCrashThread)NULL;
...
sentrycrash__unset_async_caller(sentrycrash_async_backtrace_t *backtrace)
{
    ...
    if (__atomic_load_n(&slot->thread, __ATOMIC_ACQUIRE) == thread) {
        ...
        __atomic_store_n(&slot->thread, (BuzzSentryCrashThread)NULL, __ATOMIC_RELEASE);
    }
