...
@implementation BuzzSentryCrashDoctorParam
...
@interface BuzzSentryCrashDoctorFunctionCall : NSObject
...
@implementation BuzzSentryCrashDoctorFunctionCall
...
- (NSString *)descriptionForObjCCall
{
    ...
    BuzzSentryCrashDoctorParam *receiverParam = [self.params objectAtIndex:0];
    ...
    BuzzSentryCrashDoctorParam *selectorParam = [self.params objectAtIndex:1];
    if (![selectorParam.type isEqualToString:@BuzzSentryCrashMemType_String]) {
        return nil;
    }
    ...
    for (int paramNum = 0; paramNum < paramCount; paramNum++) {
        [string appendString:@":"];
        if (paramNum < 2) {
            BuzzSentryCrashDoctorParam *param = [self.params objectAtIndex:(NSUInteger)paramNum + 2];
            if (param.value != nil) {
                if ([param.type isEqualToString:@BuzzSentryCrashMemType_String]) {
...
- (NSString *)descriptionWithParamCount:(int)paramCount
{
    ...
    for (int i = 0; i < paramCount; i++) {
        BuzzSentryCrashDoctorParam *param = [self.params objectAtIndex:(NSUInteger)i];
...
@implementation BuzzSentryCrashDoctor
...
{
    return [report objectForKey:@BuzzSentryCrashField_RecrashReport];
}

- (NSDictionary *)systemReport:(NSDictionary *)report
{
    return [report objectForKey:@BuzzSentryCrashField_System];
}

- (NSDictionary *)crashReport:(NSDictionary *)report
{
    return [report objectForKey:@BuzzSentryCrashField_Crash];
}

- (NSDictionary *)infoReport:(NSDictionary *)report
{
    return [report objectForKey:@BuzzSentryCrashField_Report];
}

- (NSDictionary *)errorReport:(NSDictionary *)report
{
    return [[self crashReport:report] objectForKey:@BuzzSentryCrashField_Error];
}

- (CPUFamily)cpuFamily:(NSDictionary *)report
{
    NSDictionary *system = [self systemReport:report];
    NSString *cpuArch = [system objectForKey:@BuzzSentryCrashField_CPUArch];
...
- (NSString *)mainExecutableNameForReport:(NSDictionary *)report
{
    ...
    return [info objectForKey:@BuzzSentryCrashField_ProcessName];
}

- (NSDictionary *)crashedThreadReport:(NSDictionary *)report
{
    NSDictionary *crashReport = [self crashReport:report];
    NSDictionary *crashedThread = [crashReport objectForKey:@BuzzSentryCrashField_CrashedThread];
    if (crashedThread != nil) {
        return crashedThread;
    }

    for (NSDictionary *thread in [crashReport objectForKey:@BuzzSentryCrashField_Threads]) {
        if ([[thread objectForKey:@BuzzSentryCrashField_Crashed] boolValue]) {
            return thread;
        }
    }
    return nil;
...
{
    NSDictionary *backtrace = [threadReport objectForKey:@BuzzSentryCrashField_Backtrace];
    return [backtrace objectForKey:@BuzzSentryCrashField_Contents];
}

- (NSDictionary *)basicRegistersFromThreadReport:(NSDictionary *)threadReport
{
    NSDictionary *registers = [threadReport objectForKey:@BuzzSentryCrashField_Registers];
    NSDictionary *basic = [registers objectForKey:@BuzzSentryCrashField_Basic];
    return basic;
}
...
- (NSDictionary *)lastInAppStackEntry:(NSDictionary *)report
{
    ...
    for (NSDictionary *entry in backtrace) {
        NSString *objectName = [entry objectForKey:@BuzzSentryCrashField_ObjectName];
...
{
    NSDictionary *machError = [errorReport objectForKey:@BuzzSentryCrashField_Mach];
    if (machError != nil) {
        NSString *exceptionName = [machError objectForKey:@BuzzSentryCrashField_ExceptionName];
        return [exceptionName isEqualToString:@"EXC_BAD_ACCESS"];
    }
    NSDictionary *signal = [errorReport objectForKey:@BuzzSentryCrashField_Signal];
    NSString *sigName = [signal objectForKey:@BuzzSentryCrashField_Name];
...
{
    NSDictionary *crashedThread = [self crashedThreadReport:report];
    NSArray *notableAddresses = [crashedThread objectForKey:@BuzzSentryCrashField_NotableAddresses];
    for (NSDictionary *address in [notableAddresses objectEnumerator]) {
        NSString *type = [address objectForKey:@BuzzSentryCrashField_Type];
        if ([type isEqualToString:@"string"]) {
            NSString *value = [address objectForKey:@BuzzSentryCrashField_Value];
    ...
    for (NSDictionary *entry in backtrace) {
        NSString *objectName = [entry objectForKey:@BuzzSentryCrashField_ObjectName];
        NSString *symbolName = [entry objectForKey:@BuzzSentryCrashField_SymbolName];
...
- (BuzzSentryCrashDoctorFunctionCall *)lastFunctionCall:(NSDictionary *)report
{
    BuzzSentryCrashDoctorFunctionCall *function = [[BuzzSentryCrashDoctorFunctionCall alloc] init];
    NSDictionary *lastStackEntry = [self lastStackEntry:report];
    function.name = [lastStackEntry objectForKey:@BuzzSentryCrashField_SymbolName];

    NSDictionary *crashedThread = [self crashedThreadReport:report];
    NSDictionary *notableAddresses =
        [crashedThread objectForKey:@BuzzSentryCrashField_NotableAddresses];
    ...
    for (NSString *regName in regNames) {
        BuzzSentryCrashDoctorParam *param = [[BuzzSentryCrashDoctorParam alloc] init];
        ...
        if (notableAddress == nil) {
            ...
            param.type = [notableAddress objectForKey:@BuzzSentryCrashField_Type];
            NSString *className = [notableAddress objectForKey:@BuzzSentryCrashField_Class];
            NSString *previousClass =
                [notableAddress objectForKey:@BuzzSentryCrashField_LastDeallocObject];
            NSString *value = [notableAddress objectForKey:@BuzzSentryCrashField_Value];

            if ([param.type isEqualToString:@BuzzSentryCrashMemType_String]) {
                param.value = value;
            } else if ([param.type isEqualToString:@BuzzSentryCrashMemType_Object]) {
                param.className = className;
                param.isInstance = YES;
            } else if ([param.type isEqualToString:@BuzzSentryCrashMemType_Class]) {
                param.className = className;
                param.isInstance = NO;
            }
...
- (NSString *)zombieCall:(BuzzSentryCrashDoctorFunctionCall *)functionCall
{
...
{
    NSDictionary *stack = [crashedThreadReport objectForKey:@BuzzSentryCrashField_Stack];
    return [[stack objectForKey:@BuzzSentryCrashField_Overflow] boolValue];
}
...
- (NSString *)diagnoseCrash:(NSDictionary *)report
{
    @try {
        NSString *lastFunctionName =
            [[self lastInAppStackEntry:report] objectForKey:@BuzzSentryCrashField_SymbolName];
        ...
        NSString *crashType = [errorReport objectForKey:@BuzzSentryCrashField_Type];
        if ([crashType isEqualToString:@BuzzSentryCrashExcType_NSException]) {
            NSDictionary *exception = [errorReport objectForKey:@BuzzSentryCrashField_NSException];
            NSString *name = [exception objectForKey:@BuzzSentryCrashField_Name];
            NSString *reason = [exception objectForKey:@BuzzSentryCrashField_Reason]
                ? [exception objectForKey:@BuzzSentryCrashField_Reason]
                : [errorReport objectForKey:@BuzzSentryCrashField_Reason];
        ...
        BuzzSentryCrashDoctorFunctionCall *functionCall = [self lastFunctionCall:report];
        ...
        if ([self isInvalidAddress:errorReport]) {
            uintptr_t address = (uintptr_t)[
                [errorReport objectForKey:@BuzzSentryCrashField_Address] unsignedLongLongValue];
            if (address == 0) {
        ...
        if (symbols) {
            return [NSString stringWithFormat:@"No diagnosis due to exception %@:\n%@\nPlease "
                                              @"file a bug report to the BuzzSentryCrash project.",
                             e, symbols];
        }
        return [NSString stringWithFormat:@"No diagnosis due to exception %@\nPlease file a "
                                          @"bug report to the BuzzSentryCrash project.",
                         e];
    }
}
