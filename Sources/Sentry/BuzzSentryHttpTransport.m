...
@property (nonatomic, strong) BuzzSentryReachability *reachability;
...
- (void)recordLostEvent:(BuzzSentryDataCategory)category reason:(BuzzSentryDiscardReason)reason
{
    ...
    NSString *key = [NSString stringWithFormat:@"%@:%@", nameForBuzzSentryDataCategory(category),
                              nameForBuzzSentryDiscardReason(reason)];
...
{
    [self recordLostEvent:dataCategory reason:kBuzzSentryDiscardReasonRateLimitBackoff];
}
...
{
    [self recordLostEvent:dataCategory reason:kBuzzSentryDiscardReasonCacheOverflow];
}
...
{
    __block BuzzSentryHttpTransport *_self = self;
    [self.requestManager
               addRequest:request
...
- (void)recordLostEventFor:(NSArray<BuzzSentryEnvelopeItem *> *)items
{
    for (BuzzSentryEnvelopeItem *item in items) {
        ...
        BuzzSentryDataCategory category = BuzzSentryDataCategoryForEnvelopItemType(itemType);
        [self recordLostEvent:category reason:kBuzzSentryDiscardReasonNetworkError];
    }
}
