...
class BuzzSentryFileIOTrackingIntegrationTests: XCTestCase {
    ...
    func test_WritingTrackingDisabled_forIOOption() {
        BuzzSentrySDK.start(options: fixture.getOptions(enableFileIOTracking: false))
        
        assertWriteWithNoSpans()
    }
    
    func test_WritingTrackingDisabled_forSwizzlingOption() {
        BuzzSentrySDK.start(options: fixture.getOptions(enableSwizzling: false))
        
        assertWriteWithNoSpans()
    }
    
    func test_WritingTrackingDisabled_forAutoPerformanceTrackingOption() {
        BuzzSentrySDK.start(options: fixture.getOptions(enableAutoPerformanceTracking: false))
        
        assertWriteWithNoSpans()
    }
    
    func test_WritingTrackingDisabled_TracingDisabled() {
        BuzzSentrySDK.start(options: fixture.getOptions(tracesSampleRate: 0))
        
        assertWriteWithNoSpans()
    }
    
    func test_Writing_Tracking() {
        BuzzSentrySDK.start(options: fixture.getOptions())
    ...
    private func assertSpans( _ spansCount: Int, _ operation: String, _ description: String = "TestFile", _ block: () -> Void) {
        let parentTransaction = BuzzSentrySDK.startTransaction(name: "Transaction", operation: "Test", bindToScope: true)
