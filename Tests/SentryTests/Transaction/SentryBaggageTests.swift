...
class BuzzSentryBaggageTests: XCTestCase {
    func test_baggageToHeader() {
        let header = BuzzSentryBaggage(trace: SentryId.empty, publicKey: "publicKey", releaseName: "release name", environment: "teste", transaction: "transaction", userSegment: "test user", sampleRate: "0.49").toHTTPHeader()
    ...
    func test_baggageToHeader_AppendToOriginal() {
        let header = BuzzSentryBaggage(trace: SentryId.empty, publicKey: "publicKey", releaseName: "release name", environment: "teste", transaction: "transaction", userSegment: "test user", sampleRate: "0.49").toHTTPHeader(withOriginalBaggage: ["a": "a", "sentry-trace_id": "to-be-overwritten"])
    ...
    func test_baggageToHeader_onlyTrace_ignoreNils() {
        let header = BuzzSentryBaggage(trace: SentryId.empty, publicKey: "publicKey", releaseName: nil, environment: nil, transaction: nil, userSegment: nil, sampleRate: nil).toHTTPHeader()
